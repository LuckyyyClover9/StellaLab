...
fds3 = fileDatastore('*conditions.xlsx','ReadFcn', @importdata);
fds4 = fileDatastore('2AG_100+MAFP_100.xlsx','ReadFcn', @importdata);
...
Ddatas{j} = zeros(91,cdnsnumber);                                                                       
            ...
            for i = 1:length(cell_dat_split)
                ...
                if sum(all(myDatas(:,idnx) ~= 0)) == lengthIdnx            % If the sum of myDatas at this finalIdnx == 4 then proceed
                    ...
                    Dsize = 91-max(size(myDatas));                         %So if the data has less than cnm columns == 0, as in it has data for DMSO and another conditons
                    for i = 1:lengthIdnx
                        indexx = idnx(i);
...
avgDatas = reshape(myData,91,cdnsnumber,[]);
...
avgDatas2 = reshape(myData2,91,cdnsnumber,[]);
...
%% 0-30 MIN
zeroMin = ismember(Time2, datetime('00:00:00'));
zeroMin2 = find(zeroMin, 1);
thirtyMin = ismember(Time2, datetime('00:30:00'));
thirtyMin2 = find(thirtyMin, 1);
moaTable = drTable(zeroMin2:thirtyMin2,:);
%% 6-9 MIN
...
%% 0-30
avgResponse = table(mean(basalDataTable{zeroMin2:thirtyMin2,:},1)');
avgResponse.Properties.VariableNames = {'Mean Basil'};

meanSEM = table(mean(SEMtable{zeroMin2:thirtyMin2,:},1)');
meanSEM.Properties.VariableNames = {'Mean(SEM)'};
...
MOATable = [cdnsMOA avgResponse meanSEM n];
writetable(MOATable, strcat(title_name,'_','0-30-Min_', datetoday,'.xlsx'));
%% 4-5
